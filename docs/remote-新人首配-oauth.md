...
- **网关域名**：形如 `https://<域名>`（例如 `https://d1234abcd56ef.cloudfront.net`）
...
> ⚠️ **填完先自查域名**（最常见的翻车点）：MCP endpoint、Authorization URL、Token URL 三栏的域名必须**逐字一致**。CloudFront 域名是一串随机字符，手打极易看错形近字母（如 `y`/`v`、`0`/`o`）。**务必复制粘贴，别手打**——错一个字母授权页就打不开（浏览器报"意外终止了连接"）。
