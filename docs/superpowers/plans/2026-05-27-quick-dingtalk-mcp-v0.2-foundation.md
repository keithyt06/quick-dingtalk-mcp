...
### Task 3: PoC 调研 — dws token 注入（D2 优先）

按 spec §8.2，先探 D2（dws 是否暴露 import/restore/from-token 子命令），其次 D1（逆向 file-DEK 加密格式）。**Plan 1 只做调研，不写实现**。产出 `docs/superpowers/notes/2026-05-27-poc-token-injection.md`，给 Plan 2 接力。
...
- `DINGTALK_DWS_AGENTCODE=<id>` → 触发 host-owned PAT 模式，权限错走 stderr JSON + exit=4
````
...
git commit -m "docs(qdm): PoC 调研记录 — dws token 注入方案 D2/D1 (Plan 2 接力)"
```

---

### Task 4: shared — scope-map.json + tier1.json（手写）

scope-map 把 dws 命令路径映射到钉钉 OpenAPI scope 名（错误重写需要）；tier1 列 30 个工具名 + 6 个 v0.1 alias 映射。两份都是手写 JSON。

**Files:**
- Create: `packages/shared/scope-map.json`
- Create: `packages/shared/tier1.json`

- [ ] **Step 1: 写 `scope-map.json`**

按 spec §4.2 的域分布列 30 个 tier1 命令对应的 scope。**字段**：key 是 dws canonical path（点号分隔），value 是 scope 字符串数组。

```json
{
  "_version": "1",
  "_note": "scope names per https://open.dingtalk.com/document/ — Plan 2 dispatch errors will look up by command path",
  "chat.message.send": ["IM.Message.Send"],
  "chat.message.list": ["IM.Message.Read"],
  "chat.message.search": ["IM.Message.Read"],
  "chat.message.recall": ["IM.Message.Send"],
  "chat.message.list-topic-replies": ["IM.Message.Read"],
  "chat.message.send-card": ["IM.Message.Send"],
  "chat.message.list-at-me": ["IM.Message.Read"],
  "chat.message.mark-read": ["IM.Message.Send"],
  "contact.user.search": ["Contact.User.Read"],
  "contact.user.get-self": ["Contact.User.Read"],
  "contact.user.get": ["Contact.User.Read"],
  "contact.dept.list": ["Contact.Department.Read"],
  "contact.dept.user-list": ["Contact.User.Read"],
  "chat.search": ["IM.Chat.Read"],
  "chat.create": ["IM.Chat.Create"],
  "chat.member.list": ["IM.Chat.Read"],
  "chat.join": ["IM.Chat.Join"],
  "calendar.event.list": ["Calendar.Event.Read"],
  "calendar.event.create": ["Calendar.Event.Write"],
  "calendar.event.update": ["Calendar.Event.Write"],
  "calendar.event.attendee-list": ["Calendar.Event.Read"],
  "drive.file.list": ["Drive.File.Read"],
  "drive.file.create": ["Drive.File.Write"],
  "drive.doc.read": ["Drive.Doc.Read"],
  "drive.doc.append": ["Drive.Doc.Write"],
  "todo.list": ["Todo.Read"],
...
> **注**：scope 名在 plan 2 阶段对照 dws 实际 PAT 错误响应可能要修。Plan 1 先写一版骨架，Task 9 dispatcher 测试需要它就够。
...
Run: `node -e 'const t = require("./packages/shared/tier1.json"); console.log("tools:", t.tools.length, "aliases:", Object.keys(t.aliases).length)'`
Expected: `tools: 30 aliases: 6`

- [ ] **Step 4: Commit**

```bash
git add packages/shared/scope-map.json packages/shared/tier1.json
git commit -m "feat(shared): add scope-map.json (30 entries) and tier1.json (30 tools + 6 v0.1 aliases)"
```
...
test("isPATExitCode: 4 → true; 0/1/2 → false", () => {
  assert.equal(isPATExitCode(4), true);
  assert.equal(isPATExitCode(0), false);
  assert.equal(isPATExitCode(1), false);
});

test("parsePATError: 解结构化 JSON", () => {
  const r = parsePATError(stderrJSON);
  assert.equal(r.error, "permission_required");
  assert.deepEqual(r.missing_scopes, ["Contact.User.Read", "Calendar.Event.Write"]);
});
...
test("rewritePAT mode=local: 输出 dws pat chmod 提示", () => {
  const out = rewritePAT(parsePATError(stderrJSON), { mode: "local" });
  assert.match(out.message, /dws pat chmod Contact\.User\.Read Calendar\.Event\.Write/);
  assert.deepEqual(out.missing_scopes, ["Contact.User.Read", "Calendar.Event.Write"]);
});

test("rewritePAT mode=remote: 输出授权 URL", () => {
  const out = rewritePAT(parsePATError(stderrJSON), {
    mode: "remote",
    authorizeUrlBuilder: (scopes) =>
      `https://auth.example.com/authorize?extra_scope=${scopes.join(",")}&t=hmac2`,
  });
  assert.match(out.authorize_url, /authorize\?extra_scope=Contact\.User\.Read,Calendar\.Event\.Write/);
