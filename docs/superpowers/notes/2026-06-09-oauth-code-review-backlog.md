...
### #1 服务端 TTL 未校验(只靠 DynamoDB best-effort TTL)— 优先级高
...
### #2 OAuthStateTable `removalPolicy: DESTROY` + 无 PITR,却存了持久记录 — 优先级高
...
### #4 refresh_token grant 仅在调用方主动带 client_id 时才校验绑定
...
### #5 IRREVERSIBLE_VERBS 与 DESTRUCTIVE_VERBS 两份清单不一致
