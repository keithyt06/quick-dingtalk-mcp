...
case "$SUB" in
  ...
  refresh)
    ...
    LAMBDA_ARN=$(aws cloudformation describe-stacks --stack-name QdmRemoteOAuth --region "$REGION" \
      --query 'Stacks[0].Outputs[?OutputKey==`TokenRefreshShimArn`].OutputValue' --output text)
    aws lambda invoke --region "$REGION" --function-name "$LAMBDA_ARN" \
  ...
  logs)
    ...
    aws logs tail "/aws/lambda/QdmRemoteOAuth-$LAMBDA*" --follow --region "$REGION"
    ;;
  *)
